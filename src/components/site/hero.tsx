    ...
    >
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-3">
          <DashboardCard title="Revenue" icon={<Activity className="size-3.5" />} float={0.2}>
          ...
          <DashboardCard title="Inventory" icon={<Boxes className="size-3.5" />} float={0.9}>
            <p className="font-display text-2xl font-semibold">
              <Counter to={99.4} suffix="%" decimals={1} />
            </p>
            <p className="mt-1 text-xs text-muted-foreground">Accuracy · 9 warehouses</p>
          ...
          <DashboardCard title="Orders" icon={<Activity className="size-3.5" />} float={1.4}>
            <p className="font-display text-2xl font-semibold">
              <Counter to={1284} />
            </p>
            <Bars />
          ...
          <DashboardCard title="AI Assistant" icon={<Bot className="size-3.5" />} float={0.6}>
            <div className="space-y-2 text-xs">
              <p className="rounded-lg bg-surface-2/70 px-2.5 py-1.5 text-muted-foreground">
                Which SKUs will stock out this week?
              </p>
              <motion.p
                className="rounded-lg bg-primary/15 px-2.5 py-1.5"
                initial={{ opacity: 0, y: 6 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 1.2 }}
              >
                7 SKUs · reorder drafted for 3 suppliers.
              </motion.p>
            </div>
          </DashboardCard>

          <DashboardCard title="Cloud Status" icon={<Cloud className="size-3.5" />} float={1.1}>
            ...
            <div className="mt-2 flex gap-1">
              {Array.from({ length: 14 }).map((_, i) => (
                <span
                  key={i}
                  className={`h-5 flex-1 rounded-sm ${i === 9 ? "bg-warn/70" : "bg-signal/60"}`}
                />
              ))}
            </div>
          </DashboardCard>

          <DashboardCard title="Automation" icon={<Workflow className="size-3.5" />} float={1.7}>
            <p className="font-display text-2xl font-semibold">
              <Counter to={38} suffix="k" />
            </p>
            <p className="mt-1 text-xs text-muted-foreground">Runs this month · 0 failed</p>
            <div className="mt-3 flex items-center gap-1.5">
              {["Form", "AI", "CRM", "ERP"].map((n) => (
                <span
                  key={n}
                  className="rounded-md border border-border px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground"
                >
                  {n}
                </span>
              ))}
